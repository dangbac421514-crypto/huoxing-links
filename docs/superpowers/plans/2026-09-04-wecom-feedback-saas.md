...
Run: `corepack pnpm --dir admin type-check`
...
Run: `corepack pnpm --dir admin type-check && corepack pnpm --dir admin build-only`
...
Run: `corepack pnpm --dir admin type-check && corepack pnpm --dir admin build-only && ! rg -n 'v-html' admin/src/views/feedback`
...
Run: `corepack pnpm --dir admin add -D @playwright/test@1.62.1`
...
Run: `docker compose -f compose.test.yaml up -d --wait && corepack pnpm --dir admin exec playwright test e2e/feedback.spec.ts`
...
corepack pnpm --dir admin type-check
corepack pnpm --dir admin build-only
corepack pnpm --dir admin exec playwright test
composer audit --working-dir=serve --locked --no-interaction
corepack pnpm --dir admin audit --prod
git diff --check
git status --short
```
